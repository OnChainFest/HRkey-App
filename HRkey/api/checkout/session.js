...
import { stripe, ALLOWED_ORIGINS, allowOrigin } from "../_lib/stripe.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Ruta A: Grant gratuito para feria (no pasa por Stripe)
    if (p && p.type === "free_grant") {
      if (!email) return res.status(400).json({ error: "Email required for free activation" });
    ...
    // Si quieres aplicar el descuento desde backend:
    let discounts;
    if (p && p.type === "stripe") {
