  ...
  describe('GET /api/identity/status/:userId', () => {
    ...
    test('ID-INT-02: user A cannot view identity status of user B (403 Forbidden)', async () => {
      ...
      mockQueryBuilder.single.mockResolvedValueOnce(
        mockDatabaseSuccess(
          mockUserData({ id: 'user-a-id', email: 'usera@test.com', role: 'user' })
        )
      );
    ...
    test('ID-INT-03: user can view their own identity status (200 OK)', async () => {
      ...
      mockQueryBuilder.single
        .mockResolvedValueOnce(
          mockDatabaseSuccess(
            mockUserData({ id: 'user-own-id', email: 'own@test.com', role: 'user' })
          )
        )
        .mockResolvedValueOnce(
          mockDatabaseSuccess({
    ...
    test('ID-INT-04: superadmin can view any user identity status (200 OK)', async () => {
      ...
      mockQueryBuilder.single
        .mockResolvedValueOnce(
          mockDatabaseSuccess(
            mockUserData({ id: 'admin-id', email: 'admin@test.com', role: 'superadmin' })
          )
        )
        .mockResolvedValueOnce(
          mockDatabaseSuccess({
  ...
  describe('POST /api/identity/verify', () => {
    ...
    test('ID-INT-07: user A cannot verify identity for user B (uses own ID automatically)', async () => {
      ...
      mockQueryBuilder.single
        ...
        .mockResolvedValueOnce(
          mockDatabaseSuccess(
            mockUserData({ id: 'user-a-id', email: 'usera@test.com', identity_verified: false })
          )
        )
        .mockResolvedValueOnce(
          mockDatabaseSuccess(
            mockUserData({ id: 'user-a-id', email: 'usera@test.com', identity_verified: true })
          )
        );
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.user.id).toBe('user-a-id');
    });
    ...
    test('ID-INT-08: user can verify their own identity (200 OK)', async () => {
      ...
      mockQueryBuilder.single
        ...
        .mockResolvedValueOnce(
          mockDatabaseSuccess(
            mockUserData({ id: 'user-own-id', email: 'own@test.com', identity_verified: false })
          )
        )
        .mockResolvedValueOnce(
          mockDatabaseSuccess({
    ...
    test('ID-INT-10: should return 400 if user already verified', async () => {
      ...
      mockQueryBuilder.single
        ...
        .mockResolvedValueOnce(
          mockDatabaseSuccess(
            mockUserData({ id: 'user-id', email: 'user@test.com', identity_verified: true })
          )
        );
